...
(function() {
  angular.module("songaday").controller("AccountCtrl", function($scope, $stateParams, $firebaseArray, AccountService, SongService, TransmitService) {
    ...
    $scope.songForKey = function(song_key) {
      if (song_key && song_key.$value) {
          return SongService.get(song_key.$value);
      }
    };
  ...
  angular.module("songaday").factory("AccountService", function($rootScope, $firebaseArray, $firebaseObject, $firebaseAuth) {
    ...
    return {
      ...
      remove_song: function(song, cb) {
        ...
        song_uri = '/artists/' + me.$id + '/songs/' + song.$id;
        last_song_uri ='/artists/' + me.$id + '/last_transmission/';
        console.log(song_uri);
        song_ref = GLOBALS.FIREBASE_REF.child(song_uri);
        last_song_ref = GLOBALS.FIREBASE_REF.child(last_song_uri);
        delete me.songs[song.$id];
        var public_artist_uri = '/public_artists/' + me.$id;
        var public_artist_ref = GLOBALS.FIREBASE_REF.child(public_artist_uri);
...
(function() {
  angular.module("songaday").controller("ArtistDetailCtrl", function($scope, $stateParams, SongService, ArtistService, $firebaseAuth, $firebaseArray) {
    ...
    $scope.getSongsFromCollab = function(collaboration) {
      ...
      if (collaboration) {
        var collabRef =  GLOBALS.FIREBASE_REF.child( 'collaboration_songs').child(collaboration)
        console.log(collabRef);
        var songsRef = collabRef.child('songs');
        return $firebaseArray(songsRef);
      }
    };
...
(function() {
  angular.module("songaday").factory("TransmitService", function($rootScope, $firebaseObject, $firebaseArray, S3Uploader, ngS3Config, SongService, AccountService) {
    ...
    return {
      ...
      transmitForArtist: function(song, artist, callback) {
        var songs;
        songs = SongService.some();
        songs.$loaded(function() {
          var me = artist;
